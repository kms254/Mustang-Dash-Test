...
| Via covering | **Tented** | The board sets `m_TentViasFront` and `m_TentViasBack` true. JLC's *plugged*-via service caps at 0.5 mm and ours are 0.6 mm, so tenting is the only option that matches. **Five vias are deliberately NOT tented** — the TP1–TP5 probe points (U53). They carry an explicit not-tented override, so they survive the board-level "tented" answer; do not let anyone "fix" them back. |
