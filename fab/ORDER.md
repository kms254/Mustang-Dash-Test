...
| Via covering | **Tented** | The board sets `m_TentViasFront` and `m_TentViasBack` true. JLC's *plugged*-via service caps at 0.5 mm and ours are 0.6 mm, so tenting is the only option that matches. **Three vias are deliberately NOT tented** — the TP1 `/+5V`, TP2 `/+3V3` and TP3 `/GND` probe points (U53), so you can confirm the rails on a board that looks dead. They carry an explicit not-tented override, so they survive the board-level "tented" answer; do not let anyone "fix" them back. |
