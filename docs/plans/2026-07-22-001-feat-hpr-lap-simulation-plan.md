...
- **CAN-driven mode switching.** In the real car, TRACK/STREET will be selected by a CAN message — that is the production mechanism. The USER-button gesture added in U11 is a **bench stand-in**, not the shipping path. Deferred because the frame is not specified yet (ID, byte offset, encoding) and `dash_can.h` is still only at FDCAN loopback level, so there is nothing to receive on. When the spec exists, it needs no refactor: serial (`dash_serial.h` `DASH_CMD_MODE`), the button (U11), and CAN all converge on the same single `s->mode = ...` assignment. Add a host-testable decoder that feeds that same point.
- **RaceCapture as the CAN data source (2026-07-22 bench decision).** The intended upstream is an Autosport Labs **RaceCapture**, which broadcasts CAN and runs Lua on-device, configured from the phone app. Two things ride that:
  1. **Session length**, set in the paddock before a run group. This is what unblocks a *countdown* session timer. Countdown was rejected on 2026-07-22 for a specific reason: it must know the session length, HPDE sessions vary (20/25/30 min), and the dash has no input device — the USER button's two gestures are both spoken for (U11). Set-on-phone → CAN → dash removes that constraint entirely. Until then the session timer counts **up**, which never claims to know something it does not.
  2. **Real lap timing.** RaceCapture does GPS lap timing and predictive delta natively. Today `LAP`/`LAST`/`BEST`/`DELTA` are simulated *because there is no other source*. With RaceCapture on the bus they become measured values and the simulator's role shrinks to bench work only — a meaningfully different system from the one built here, and probably the right end state.

  **Unverified:** RaceCapture can certainly broadcast custom CAN via Lua, but how cleanly a single value can be changed *trackside* from the phone (live control vs. a config push) has not been confirmed against the actual unit. A config push is fine for a paddock workflow; confirm before designing around it.
