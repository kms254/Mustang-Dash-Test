...
- `docs/solutions/design-patterns/hand-transcribe-a-spec-multiple-times-and-gate-on-agreement.md` — the other answer to the same PDF-only-spec problem shape: where this doc diffs once against an authoritative external oracle, that one maintains N independent transcriptions whose agreement is re-proven on every test run. Pick by whether an external implementation exists to diff against.
