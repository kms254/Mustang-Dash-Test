...
Same family of trap for spatial queries: **never window-filter a board dump by an object's reference point.** Ask whether the SHAPE intersects the window, never whether some representative point sits inside it — and the correct test differs per object type: **tracks** by segment clip (Liang-Barsky), **pads and footprints** by bounding-box overlap (`GetBoundingBox()`, never `GetPosition()`), **vias** by centre±radius on *every layer they span*. Both halves are paid-for lessons: endpoint containment hid a full-width net's copper twice in one session, and centre containment then hid a decoupling cap's ground pad whose body sat outside the window while its copper reached in — a net was routed straight through it. When reviewing, treat both `inwin(sx,sy) or inwin(ex,ey)` and any range test over `pad.GetPosition()` / `fp.GetPosition()` as the bug. Note the selection effect: the more tightly a part is placed against the pin being routed, the more likely a centre filter is to miss it.
