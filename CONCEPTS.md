...
The budget is worth treating as nearly spent rather than roomy: the shipping configuration sits within a few tens of kilobytes of the ceiling, because compression choices made for image quality cost RAM_G directly. Take the current figure from the renderer that allocates it, never from a document — a stale budget figure reads as headroom that does not exist, which is the one error this concept exists to prevent.
