...
### RAM_G
EVE's fixed-size on-chip graphics memory — the home of any bitmap the chip decodes or the firmware uploads at runtime, and the fastest asset storage the renderer has. Its capacity is a hard budget that shapes asset decisions: storage formats, downscaling with render-time upscaling, and which assets are resident at once. Rendering straight from the panel's own flash once served as an escape valve for small assets, but its per-frame ceiling (see Flash Render Streaming) meant large assets needed RAM_G staging anyway, and the flash path has been retired.

How much of the budget is spent swings widely with encoding choices, because a decision made for image quality is paid for in graphics memory directly — the same layer can differ by two orders of magnitude between a block-compressed and an uncompressed form. Headroom is therefore a property of the current configuration and not of the design. Take the figure from the code that allocates it, never from a document; this entry itself once asserted the budget sat within a few tens of kilobytes of the ceiling, which was true of one configuration and badly wrong of the next. A stale budget figure reads as headroom that does not exist, which is the one error this concept exists to prevent.

*Historical caveat, no longer binding here:* the chip's on-chip PNG decoder borrows the top of RAM_G as scratch during image loads, so anything packed near the top had to leave it headroom. That shaped this project's original big-first packing order. Nothing in the firmware decodes a PNG any more, so the constraint is a property of the chip rather than a live rule.

### Asset Pack
The versioned bundle of static assets embedded in the firmware image and staged directly into RAM_G at boot. Its members are not one format: block-compressed where the asset carries detail and is drawn at native size, and uncompressed where it is smooth and magnified, because a block codec quantises a gradient per block and every boundary becomes a visible step once the asset is scaled up. The pack is verified per asset against the embedded source before the renderer may use it — a failed check skips that asset for the session. The pack formerly lived provisioned in the display module's onboard flash; that round trip was retired because the pack ships in firmware either way, staging rides the same proven command path as font loading, and boot no longer depends on the module's flash at all. Staging time is a function of the SPI Operating Point.
...
### Splash Theme
A complete visual variant of the boot splash — background treatment, accent hardware, and year mark as a matched set. The concept is still worth naming even though only one variant ships: it is what makes "the splash" a set of coordinated assets rather than a picture, and it is the unit a runtime selector would choose between.

Selection is build-time, because the cluster has no input hardware to switch with. That is a constraint on the mechanism, not on the count: retiring the extra variants was a RAM_G and flash decision, and any future runtime selection costs no extra graphics memory — only one variant is ever resident — but requires every selectable variant to be present in program storage, which is precisely the space retiring them reclaimed.
...
A dialect claims channels exclusively — no channel is decoded by two dialects — and the claim is freshness-scoped. When a claimed message goes stale, bench builds release the channel back to the simulator; car builds instead latch the claim and Dead-Front the channel, so a dying bus in the car reads as absent data rather than plausible fiction. An operator's override outranks any dialect, and a dialect outranks the simulator.

### Golden Frames
Checked-in CAN test vectors generated from a dialect's DBC by the PC toolchain: known channel values encoded into the exact bytes the wire would carry. The host test decodes them with the real firmware decoder and asserts the values round home. Their job is transcription fidelity — the spec's tables are deliberately hand-copied into several independent transcriptions (the machine-readable spec file the toolchain encodes from, the golden generator's own restatement of the scaling constants, and the firmware decoder; a separate round-trip test carries its own frame packer as one more), so a slip must reproduce identically in every one before it can hide. The vectors' expected values are derived from the spec, never from the decoder under test, and consolidating the transcriptions into one shared source would destroy the property, not clean it up.

### Bench Impostor
A PC-side program that plays a real device's CAN Dialect onto the bench bus at the device's own broadcast rates, fed by the same simulation the dash runs internally — so the firmware under test receives spec-shaped traffic indistinguishable from the car while the car isn't connected. Its output is byte-matched against the dialect's Golden Frames in a dry-run mode before an operator runs it live — a workflow discipline rather than a code-enforced gate — which keeps the impostor and the firmware decoder two independent consumers of one spec rather than a self-consistent loop.

### Dead-Front
Showing a value as absent rather than substituting anything when its source is lost. A gauge reading `--` is trustworthy; a gauge holding a last-known or simulated value is a confident lie — so car builds dead-front channels whose CAN source has gone stale, while bench builds hand them back to the simulator, because on the bench the fiction is the point.

### DLC
The length field of a CAN frame, and a code rather than a byte count: classic frames legally carry codes above eight — all meaning eight data bytes on the wire — while CAN FD reuses those same codes for payloads up to sixty-four bytes. Drivers translate the code format-blind, so receiving software must size its buffers for what the driver actually writes, and only then apply the classic meaning; treating the protocol's logical maximum as the buffer contract is how a legal-but-nonconforming frame becomes stack corruption.
...
It exists because an imported board arrives with the importing tool's factory defaults rather than the fab's limits, and measuring against those produces hundreds of findings that are all false. The staging is therefore load-bearing rather than a convenience, and it has three failure modes worth knowing. **Stage too little** and the checker cannot resolve what it needs — a missing library table yields mass "not found" findings that also mask the real ones underneath, which is why a partial staging can only be trusted at the severity it was designed for. Because staging *overwrites* the copy's values for every rule it names, **the staged set outranks the design file**: a constraint tightened in the design and not mirrored into the staged set is silently relaxed for the duration of every check. And **a staged rules file that fails to parse loads as no rules at all**, with no error on any observable channel — the checker keeps reporting normally under different law, and the only announcement is the reappearance of findings the file's waivers existed to suppress. A rules file with no waivers gives no announcement, which is why any rules-file edit is proven both ways: one rule shown firing, and the previous verdict on a known case shown unchanged.
